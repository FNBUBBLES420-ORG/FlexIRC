...
const warnedHashFallbacks = new Set();
...
function getBcrypt() {
    if (!bcryptModule) {
        try {
            ...
            error.message = `Failed to load bcrypt. Rebuild native modules for this Node.js runtime or keep USE_ARGON2=true. Original error: ${error.message}`;
            throw error;
        }
    }
...
function getArgon2() {
    if (!argon2Module) {
        try {
            ...
            error.message = `Failed to load argon2. Rebuild native modules for this Node.js runtime or switch to a fallback hasher. Original error: ${error.message}`;
            throw error;
        }
    }
...
function warnHasherFallback(algorithm, error) {
    if (warnedHashFallbacks.has(algorithm)) {
        return;
    }

    warnedHashFallbacks.add(algorithm);
    console.warn(`[auth] ${algorithm} is unavailable, falling back to built-in scrypt. ${error.message}`);
}
...
async function verifyScryptHash(password, hashedPassword) {
...
async function hashPassword(password, useArgon2) {
    if (useArgon2) {
        try {
            return await getArgon2().hash(password);
        } catch (error) {
            warnHasherFallback('argon2', error);
            return hashWithScrypt(password);
        }
    }

    try {
        return await getBcrypt().hash(password, 12);
    } catch (error) {
        warnHasherFallback('bcrypt', error);
        return hashWithScrypt(password);
    }
}

async function verifyPassword(password, hashedPassword, useArgon2) {
    if (hashedPassword.startsWith('scrypt$')) {
        return verifyScryptHash(password, hashedPassword);
    }

    if (hashedPassword.startsWith('$argon2')) {
        return getArgon2().verify(hashedPassword, password);
    }

    if (hashedPassword.startsWith('$2')) {
        return getBcrypt().compare(password, hashedPassword);
    }

    if (useArgon2) {
        try {
            return await getArgon2().verify(hashedPassword, password);
        } catch (error) {
            warnHasherFallback('argon2', error);
            return false;
        }
    ...
    try {
        return await getBcrypt().compare(password, hashedPassword);
    } catch (error) {
        warnHasherFallback('bcrypt', error);
        return false;
    }
...
class ComprehensiveIRCServer {
    ...
    setupBasicRoutes() {
        ...
        this.app.post('/shutdown', (req, res) => {
            ...
            if (!isLocalRequest) {
                return res.status(403).json({ success: false, message: 'Shutdown is only allowed from this machine.' });
            }

            res.json({ success: true, message: 'Server shutting down.' });

            setTimeout(() => {
                console.log('Shutting down from web client request...');
                process.exit(0);
            }, 250);
        });
    }
    ...
    async handleRegister(clientId, message) {
        ...
        try {
            // Hash password
            const hashedPassword = await hashPassword(password, this.config.useArgon2);
    ...
    async handleAuth(clientId, message) {
        ...
        try {
            // Verify password
            const passwordValid = await verifyPassword(password, userData.password, this.config.useArgon2);
